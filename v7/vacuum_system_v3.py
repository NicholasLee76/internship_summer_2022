...
class VacuumSystem:
    def __init__(self, chambers, pumps, valves, arms):
        ...
        for valve in self.valves:
            if isinstance(valve.get_src(), Chamber) and isinstance(valve.get_dest(), Chamber):
                ...
                temp = [valve.get_dest(), valve, valve.get_weight()]
                prev.append(temp)
                self.c_adj_dict.update({valve.get_src(): prev})

                prev = self.c_adj_dict[valve.get_dest()]
                temp = [valve.get_src(), valve, valve.get_weight()]
                prev.append(temp)
        ...
        if isinstance(edge.get_src(), Chamber) and isinstance(edge.get_dest(), Chamber):
            ...
            if check[0]:
                ...
                for key in self.c_adj_dict:  # fix this lmao so it doesnt need to run a double for loop every time it opens a valve
                    for ele in self.c_adj_dict[key]:
                        if ele[1] == edge:
                            ele[2] = True
            ...
            if check[0]:
                ...
                for key in self.p_adj_dict:  # fix this lmao so it doesnt need to run a double for loop every time it opens a valve
                    for ele in self.p_adj_dict[key]:
                        if ele[1] == edge:
                            ele[2] = True
            else:
                print(f"{name} cannot be opened: ")
    ...
    def status_react(self, pump, fix):
        if pump.get_fault():
            print(f"{pump.get_name()} has fault, closing all nearby valves")
        ...
        elif not pump.get_status():
            print(f"{pump.get_name()} is closed, closing nearby gate valves")
            for edge in self.fault_tracker[pump]:
                if edge.get_src().get_name() != 'backup' and edge.get_dest().get_name() != 'backup':
                    self.close_valve(edge)
        else:
            print(f"{pump.get_name()} is running, can open nearby valves")

    def vent(self, chamber):
        print(f"venting {chamber.get_name()}:")
        for c_list in self.c_adj_dict[chamber]:
            self.close_valve(c_list[1])
        for p_list in self.p_adj_dict[chamber]:
            if isinstance(p_list[0], Turbo):
                self.open_valve(p_list[1])
                p_list[0].set_speed(0)
                print(f"{p_list[0]} is not running")
            elif isinstance(p_list[0], Cryo):
                self.close_valve(p_list[1])
        chamber.set_pressure(760)
        print(f"{chamber} is now atm pressure")

    def pump_down(self, chamber):
        print(f"pumping down {chamber.get_name()}:")
        for c_list in self.c_adj_dict[chamber]:
            self.close_valve(c_list[1])
        for p_list in self.p_adj_dict[chamber]:
            self.open_valve(p_list[1])
            p_list[0].set_speed(1500)
            print(f"{p_list[0]} is running")
        chamber.set_pressure(10**-11)
        print(f"{chamber} is now uhv pressure")

    def chamber_react(self, chamber):
